...
Test_Performance_p_1_ordered <- Test_Performance(p=1,observed_y[-1,], Dates_test,fileNames,"OrderedLASSO_with_CV")
Test_Performance_p_2_ordered <- Test_Performance(p=2,observed_y[-1,], Dates_test,fileNames,"OrderedLASSO_with_CV")
Test_Performance_p_3_ordered <- Test_Performance(p=3,observed_y[-1,], Dates_test,fileNames,"OrderedLASSO_with_CV")
Test_Performance_p_5_ordered <- Test_Performance(p=5,observed_y[-1,], Dates_test,fileNames,"OrderedLASSO_with_CV")
...
for (i in 1:length(fileNames)){
  jpeg(paste(fileNames[i]," SE Plots with Cross Validation.jpg", sep=""))
  plot(Dates_test,log(Test_Performance_p_1[[i+2]]),col=1,type="l", ylim=c(-25,13),
       xlab = "Time",ylab = "ln(Squared Error)",main = paste(fileNames[i],"_Log Squared Errors_Test Sample_with Cross Validation"))  
  lines(Dates_test,log(Test_Performance_p_2[[i+2]]),col=2,type="l")
  lines(Dates_test,log(Test_Performance_p_3[[i+2]]),col=3,type="l")
  lines(Dates_test,log(Test_Performance_p_1_ordered[[i+2]]),col=4,type="l")
  lines(Dates_test,log(Test_Performance_p_2_ordered[[i+2]]),col=5,type="l")
  lines(Dates_test,log(Test_Performance_p_3_ordered[[i+2]]),col=6,type="l")
  lines(Dates_test,log(Test_Performance_p_5_ordered[[i+2]]),col=7,type="l")
