...
def time_vol_calc(df_single_time):
    ...
    dvol = {'Date': [df_single_time[i]['Date'][df_single_time[i]['Date'].first_valid_index()] for i in
                     range(0, len(df_single_time))], 'Volatility_Time': time_vols}
    dret = {'Date': [df_single_time[i]['Date'][df_single_time[i]['Date'].first_valid_index()] for i in
                     range(0, len(df_single_time))], 'Return_Time': time_rets}
    ...
    # da_rest = pd.DataFrame([[df_single_time[i]['Date'][df_single_time[i]['Date'].first_valid_index()] for i in
    #                          range(0, len(df_single_time))],
    #                         dret['Return_Time'],
    #                         dvol['Volatility_Time'],
    #                         [df_single_time[i]['High'][df_single_time[i]['High'].first_valid_index()] for i in
    #                          range(0, len(df_single_time))],
    #                         [df_single_time[i]['Low'][df_single_time[i]['Low'].first_valid_index()] for i in
    #                          range(0, len(df_single_time))],
    #                         [df_single_time[i]['Open'][df_single_time[i]['Open'].first_valid_index()] for i in
    #                          range(0, len(df_single_time))],
    #                         [df_single_time[i]['Close'][df_single_time[i]['Close'].first_valid_index()] for i in
    #                          range(0, len(df_single_time))]],
    #                          index=['Date', 'Return_Time', 'Volatility_Time', 'High', 'Low', 'Open', 'Close']).T

    # errdang = pd.DataFrame([dret['Date'], dvol['Volatility_Time'],da_rest])

    # we use this one below because we want the highest and lowest intraday values.
    # likewise, we want to use the open value from the beginning of the day and the close value at the end of the day
    da_rest = pd.DataFrame([[df_single_time[i]['Date'][df_single_time[i]['Date'].first_valid_index()] for i in
                             range(0, len(df_single_time))],
                            dret['Return_Time'],
                            dvol['Volatility_Time'],
                            [df_single_time[i]['High'].max(axis=0) for i in range(0, len(df_single_time))],
                            [df_single_time[i]['Low'].min(axis=0) for i in range(0, len(df_single_time))],
                            [df_single_time[i]['Open'][df_single_time[i]['Open'].first_valid_index()] for i in
                             range(0, len(df_single_time))],
                            [df_single_time[i]['Close'][df_single_time[i]['Close'].last_valid_index()] for i in
                             range(0, len(df_single_time))]],
                             index=['Date', 'Return_Time', 'Volatility_Time', 'High', 'Low', 'Open', 'Close']).T
    ...
    if inters_ret.size > 0:
        ...
        da_rest = da_rest.drop(da_rest.index[comparison_array])
        da_rest.reset_index(drop=True, inplace=True)
    # time_vol_result_zeroes and ret include the zero values...
    return time_vol_result, time_ret_result, time_vol_result_zeroes, time_ret_result_zeroes, da_rest
