...
names = ['AUDUSD.csv', 'CADUSD.csv',  'CHFUSD.csv', 'EURUSD.csv', 'GBPUSD.csv', 'JPYUSD.csv', 'NOKUSD.csv', 'NZDUSD.csv', 'SEKUSD.csv']
...
if __name__ == '__main__':
    ...
    # 
    # pool = Pool(processes=)
    # is_connected = pool.map(multip(names[0]))
    # is_connected = pool.map(multip(names[1]))
    # is_connected = pool.map(multip(names[2]))
    # is_connected = pool.map(multip(names[3]))
    # pool.close()
    # pool.join()