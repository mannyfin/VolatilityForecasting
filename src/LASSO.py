...
def lasso_regression(xmat, ymat, p, dates, alpha):
    ...
    for col in ymat.keys():
        ...
        for i_alpha in alpha:
            ...
            MSE = np.longdouble(np.mean(np.square(np.subtract(observed.iloc[-1], prediction))))
        ...
        lamda_p=alpha[MSE_list.idxmin()[0]]*len(observed)
        print('lamda '+str(lamda_p))
        print('MSE min: '+str(MSE_list.min()[0]))
        plt.show()
        print("hi")
    MSE_list = []
    ...
    #     # MSE = Performance_.mean_se(observed=observed, prediction=prediction)
    #
    #     MSE = np.sum(np.longdouble(np.mean(np.square(np.subtract(observed.iloc[-1].values.reshape(1,-1), prediction)))))
    #     # mse_sum = pd.Series(np.sum(MSE), index=['SumMSE'])
    #
    #     MSE_list.append(MSE)
    #
    ...
    # plt.xlabel('lamda')
    # plt.ylabel('MSE')
    # plt.title('LASSO training for all for p = '+str(p))
    # # the line below gives the first index of the lamda with the minimum MSE' lamda
    # lamda_p=alpha[MSE_list.idxmin()[0]]*len(observed)
    # print('lamda '+str(lamda_p))
    # print('MSE min: '+str(MSE_list.min()[0]))
