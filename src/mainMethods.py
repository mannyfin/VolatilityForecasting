...
def multiknn(names):
    ...
    dailyvol_zeroes.rename(columns={'Volatility_Time': names}, inplace=True)

    # be careful of the underscore, _
    dailyret_zeroes = pd.concat([dailyret_zeroes, daily_ret_zeroes], axis=1)

    dailyret_zeroes.rename(columns={'Return_Time': names}, inplace=True)
    ...
    # else:
    if training:
        KNN_training = [ [ fc.function_runs(dates=training_date, filename=str(names)+' Single Knn.', stringinput='Daily',warmup=warmup,input_data=test_sample,k_nn=[i])
        for i in np.arange(2,21) ] for warmup in [100]]   #200,500)]
    else:
            KNN_test.append(fc.function_runs(dates=test_date, filename=names, stringinput='Daily',warmup=100,input_data=test_sample,k_nn=[20]))
    plt.show()
    plt.close()
