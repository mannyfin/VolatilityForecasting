...
def retvoldf(ret, vol, v):
    ...
    combined = pd.concat([vol.Date[1:-1], ret.Return_Time[1:-1], vol.Volatility_Time[:-2], vol.Volatility_Time[:-1]
                         .shift(-1), vol.Volatility_Time[:].shift(-2)], axis=1).dropna().set_index('Date')
    preprocess = v.join(combined, how='inner')
    preprocess.columns = ['V', 'ret_now', 'vol_past', 'vol_now', 'vol_future']
    # make label column
    preprocess['label'] = 0
