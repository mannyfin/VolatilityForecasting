...
def lin_reg_comb(data, n, warmup_period, name=None, test=False):
    ...
    for initial in range(warmup_period, LogVol.last_valid_index()[0]):
        for i in range(n):
        ...
        SE = (A.predict(LogVol.loc[initial-n : initial].values.reshape(1, -1)) - LogVol.loc[initial]) ** 2
